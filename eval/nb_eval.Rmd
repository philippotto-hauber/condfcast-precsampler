...
df_eval_models %>% 
  ...
  filter(horizon >= 0, model == "Nr2_Nj1_Np2_Ns0") %>% 
  pivot_wider(names_from = "type", values_from = "sfe") %>% 
  group_by(mnemonic, model, horizon) %>% 
  summarise(dm_pval = round(dm_test(conditional_hard, unconditional), digits = 2)) -> tmp 

tmp %>% 
  left_join(unique(select(df_eval, mnemonic, category)), by = "mnemonic") -> tmp2

tmp2 %>% 
  group_by(category, horizon, model) %>% 
  mutate(is_significant = ifelse(dm_pval <= 0.10, 1, 0)) -> tmp3
...
tmp4 %>% 
  ...
  pivot_wider(names_from = "horizon", values_from = "result") -> tmp5
```
