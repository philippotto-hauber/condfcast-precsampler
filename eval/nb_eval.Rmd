...
calc_relative_forecast_accuracy <- function(df_eval_models, df_eval_benchmark, str_model, subsample)
{
  ...
  summarise(rmsfe = sqrt(mean(sfe)), 
            mean_logs = mean(logs),
            mean_crps = mean(crps)) -> df_eval_models_avg

  df_eval_benchmark %>% 
  group_by(series, horizon) %>%
  summarise(rmsfe = sqrt(mean(sfe)), 
            mean_logs = mean(logs),
            mean_crps = mean(crps)) -> df_eval_benchmark_avg
  ...
  # select columns
  df_eval <- select(df_eval, mnemonic, horizon, type, rel_rmsfe, rel_logs, rel_crps)
