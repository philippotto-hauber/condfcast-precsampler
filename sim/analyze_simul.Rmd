...
dat %>% group_by(mod, sampler, ftype) %>%
...
dat %>% 
  ...
  facet_grid(Nh~Ncond, scales = "free_y")+
...
dat %>% 
  ...
  ggplot(aes(x = mod, y = value_rel, 
             color = sampler, shape = ftype, group = ftype))+
  ...
  scale_shape(solid = FALSE)+
  scale_color_jco()+
  scale_y_continuous(trans='log10', breaks = c(0.2, 1, 2, 10, 50))+
